...
# TAB 3: Marketing Mix Modeling
    ...
    if not st.session_state.data_uploaded:
        ...
        with param_col1:
            adstock_alpha = st.slider("Adstock Rate (α)", 0.0, 0.9, 0.5, 0.05, help="Carryover effect")
        
        with param_col2:
            hill_slope = st.slider("Hill Slope", 0.5, 2.0, 1.0, 0.1, help="Saturation curve shape")
        
        with param_col3:
            train_test_split = st.slider("Train/Test Split", 0.6, 0.9, 0.8, 0.05, help="Proportion for training")
        ...
        if st.button("🚀 Run Marketing Mix Model", type="primary", use_container_width=True):
            with st.spinner("Training Marketing Mix Model..."):
                try:
                    ...
                    st.info("Step 4/7: Engineering media features...")
